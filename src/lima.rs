...
const PRIVATE_EGRESS_BUILDER_NAME: &str = "coop-egress-image-builder";
const PRIVATE_EGRESS_IMAGE_VERSION: u32 = 1;
...
fn limactl_command(private_egress: bool) -> Command {
...
pub fn create_and_start(
    ...
    // A user may enable private egress after building the golden image. The
    // image is still reusable, but its shared Lima template must be refreshed
    // so every newly-created instance has only the user-v2 network.
    if cfg.private_egress.is_some() {
        generate_start_template(cfg, &inst.image)?;
    }
    ...
    let mut command = limactl_command(cfg.private_egress.is_some());
...
fn private_egress_image_paths(cfg: &CoopConfig, image: &ImageName) -> (PathBuf, PathBuf) {
    let dir = cfg.image_dir(image);
    (
        dir.join("lima-private-egress-base.img"),
        dir.join("lima-private-egress-base.stamp"),
    )
}

fn private_egress_source_fingerprint(source: &Path) -> Result<String> {
    let metadata = fs::metadata(source)
        .with_context(|| format!("Failed to inspect source image {}", source.display()))?;
    let modified = metadata
        .modified()
        .context("Source image has no modification timestamp")?
        .duration_since(UNIX_EPOCH)
        .context("Source image timestamp predates the Unix epoch")?;
    Ok(format!(
        "version={PRIVATE_EGRESS_IMAGE_VERSION}\nsource_bytes={}\nsource_mtime_ns={}\nguard_sha256={}\n",
        metadata.len(),
        modified.as_nanos(),
        Sha256Hash::of(crate::private_egress::boot_guard_install_script())
    ))
}

/// Build a derivative of the ordinary golden image with an early-boot OUTPUT
/// guard already enabled. The temporary builder has no workspace, credentials,
/// or user mounts; only agent instances are required to be fail-closed from
/// their first packet.
fn ensure_private_egress_base(cfg: &CoopConfig, image: &ImageName) -> Result<PathBuf> {
    let source = cfg
    ...
    if guarded.exists() && fs::read_to_string(&stamp).is_ok_and(|existing| existing == fingerprint)
    {
        return guarded
            .canonicalize()
            .context("Failed to resolve private-egress base image");
    }

    tracing::info!("Building early-boot guarded image for private egress");
    let template = cfg
        .data_dir
        .join("private-egress")
        .join("image-builder.yaml");
    let yaml = private_egress_builder_yaml(cfg, &source);
    crate::fs_util::atomic_write_with_mode(&template, &yaml, 0o600)?;
    build_private_egress_base(&template, &guarded, &stamp, &fingerprint)?;
    guarded
        .canonicalize()
        .context("Failed to resolve newly built private-egress base image")
}

fn private_egress_builder_yaml(cfg: &CoopConfig, source: &Path) -> String {
    let arch = if cfg!(target_arch = "aarch64") {
        "aarch64"
    } else {
        "x86_64"
    };
    let disk_gib = base_image_size_gib(source).unwrap_or(cfg.vm.template_size_gib);
    format!(
        r#"# Generated by coop. Contains no credentials or user mounts.
vmType: "vz"
...
"#,
        ...
        disk_gib = disk_gib.as_u32(),
    )
}

fn build_private_egress_base(
    template: &Path,
    guarded: &Path,
    stamp: &Path,
    fingerprint: &str,
) -> Result<()> {
    let _ = limactl_command(true)
        .args(["delete", "--force", PRIVATE_EGRESS_BUILDER_NAME])
        .output();
    let staging = guarded.with_extension("img.new");
    if staging.exists() {
        fs::remove_file(&staging)
            .with_context(|| format!("Failed to remove stale {}", staging.display()))?;
    }
    let result = (|| -> Result<()> {
        limactl_command(true)
            .arg("start")
            .arg(template)
            .arg(format!("--name={PRIVATE_EGRESS_BUILDER_NAME}"))
            .arg("--tty=false")
            .status()
            .context("Failed to start private-egress image builder")?
            .success()
            .then_some(())
            .context("Private-egress image builder failed to start")?;

        let mut installer = crate::cmd::Cmd::new("limactl");
        for name in PROXY_ENV_VARS {
            installer = installer.env_remove(name);
        }
        installer
            .args([
                "shell",
                PRIVATE_EGRESS_BUILDER_NAME,
                "--",
                "sudo",
                "sh",
                "-s",
            ])
            .stdin_input(crate::private_egress::boot_guard_install_script().to_vec())
            .run()
            .context("Failed to install early-boot guard in derivative image")?;
        ...
        limactl_command(true)
            ...
            .status()
            .context("Failed to clean cloud-init in derivative image")?
            .success()
            .then_some(())
            .context("cloud-init clean failed in derivative image")?;

        limactl_command(true)
            .args(["stop", PRIVATE_EGRESS_BUILDER_NAME])
            .status()
            .context("Failed to stop private-egress image builder")?
            .success()
            .then_some(())
            .context("Private-egress image builder failed to stop")?;

        let lima_dir = lima_home()?.join(PRIVATE_EGRESS_BUILDER_NAME);
        let disk = if lima_dir.join("disk").exists() {
            lima_dir.join("disk")
        } else {
            lima_dir.join("diffdisk")
        };
        fs::copy(&disk, &staging).with_context(|| {
            format!(
                "Failed to copy guarded disk {} -> {}",
                disk.display(),
                staging.display()
            )
        })?;
        fs::rename(&staging, guarded).with_context(|| {
            format!(
                "Failed to activate guarded image {} -> {}",
                staging.display(),
                guarded.display()
            )
        })?;
        crate::fs_util::atomic_write_with_mode(stamp, fingerprint, 0o600)
    })();
...
"#,
        ...
        proxy_policy = if cfg.private_egress.is_some() {
            "propagateProxyEnv: false"
        } else {
            ""
        },
        network = if cfg.private_egress.is_some() {
            "\nnetworks:\n- lima: user-v2\n"
        } else {
            ""
        },
        rosetta_enabled = cfg.private_egress.is_none(),
    );
...
mod tests {
    ...
    fn private_egress_fingerprint_tracks_source_and_guard_version() {
        ...
        let first = private_egress_source_fingerprint(&image).unwrap();
        assert!(first.contains("version=1\n"));
        assert!(first.contains("source_bytes=5\n"));
        assert!(first.contains("guard_sha256="));

        fs::write(&image, b"second-version").unwrap();
        let second = private_egress_source_fingerprint(&image).unwrap();
        assert_ne!(first, second);
    }

    #[test]
    fn private_egress_builder_has_no_user_mounts_or_proxy_forwarding() {
        let cfg = CoopConfig::default();
        let yaml = private_egress_builder_yaml(&cfg, Path::new("/private/base.img"));
        assert!(yaml.contains("propagateProxyEnv: false"));
