...
**Don't bother with:** `backend.rs`, `lima.rs`, `setup.rs`, `update.rs`, `shell.rs`, `port_forward.rs`, `cmd.rs`, `ssh.rs`, `vm.rs`, `main.rs`, and — inside `src/commands/` — the `cmd_*` dispatch entrypoints and the handlers that take a `&PlatformBackend` or open a TTY prompt (e.g. `create_up_instance`, `restart_instance`, `find_stopped_instance`, `resolve_running`, `resolve_devcontainer`, `purge_all_data`), plus the `lib.rs` `run`/`init_tracing` shims. These mostly shell out, run SSH, or talk to external services — unit tests can't catch behavioral changes there. `tests/integration.sh` does that job. This list is enforced (not just advised) by `.cargo/mutants.toml` — see **Scoping** below.
...
- **The whole-module "Don't bother with" files above** (and `main.rs`), via `exclude_globs` — they only shell out, drive SSH, or hit external services.
